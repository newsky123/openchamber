...
- `packages/web/server/lib/prompt-telemetry/runtime.js`: `OPENCHAMBER_PROMPT_TELEMETRY`-gated managed OpenCode plugin that appends the system prompt, messages, tool schemas, and sampling parameters of each model call to local JSONL. Appended last so its hooks observe what the other managed plugins leave behind.
